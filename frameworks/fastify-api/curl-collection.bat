...
REM ===== Base URL =====
set BASE_URL=http://localhost:3001/tasks

echo.
echo ===== FASTIFY TASKS API COLLECTION =====
echo.
...
REM ===== CREATE NEW TASK =====
echo == POST /tasks ==
curl -s -X POST %BASE_URL% -H "Content-Type: application/json" -d "{\"title\":\"Learn Fastify and MongoDB\",\"completed\":false}" > response.json
type response.json
echo.
...
echo ===== FASTIFY COLLECTION FINISHED =====
pause
